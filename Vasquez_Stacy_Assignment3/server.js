...
A3 - fixed login errors from A2, referenced from WW3 school, Youtube, Flavoscopes, A3 examples, A3 workshop, Kylie Dee and Sharon Diep
...
app.post("/process_login", function(request, response) { // process login from POST and redirect to invoice if matches correctly
    ...
    if (typeof users_reg_data[the_username] != 'undefined') { //ask the object if it has matching username or leaving it as undefined
        if (users_reg_data[the_username].password == POST.password) {
            ...
            session.username = the_username //making universal refernce  so the_username will be the sessions username stored w cookie, idea from kylie dee
...
`;
    response.send(str);
}); // console logs for registration referenced from sharon diep
...
app.get("/display_cart", function(request, response, next) { //created to display items in the shopping cart
    ...
    var str = "";
    // referenced modification buttons from https://www.youtube.com/watch?v=h9MIZN570r8 
...
<br>
  ...
  `;
    ...
    // Compute shipping
    if (subtotal > 0 && subtotal <= 2500) { // If subtotal is less than or equal to 2.5k, shipping is 5
        shipping = 5;
    } else if (subtotal > 2500 && subtotal <= 5000) { // Else if subtotal is less than or equal to 5k shipping is 10
        shipping = 10;
    } else if (subtotal > 5000) { // Else if subtotal is greater than 5k, shipping is (free)
...
app.post("/process_form", function(request, response) { // adding data collected to cart if valid
    POST = request.body;
    RS = request.session
    ...
    if (isValidData == true && hasPurchases == true) { // if the quantity is a valid, add to cart
        RS.cart[product_type] = POST; // request the body of sessions cart data by product type
        qString += "&addedToCart=true"; // add to qString a string in URL that identifies that something valid has been added to the cart
        console.log(RS.cart); // log the sessions of added items into the console
    }
