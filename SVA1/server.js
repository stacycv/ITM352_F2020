...
const user_data_info = 'user_data.json'; // new
const fs = require('fs'); //Loading file system
    ...
    app.post("/process_login", function(request, response) {
        ...
        if (typeof user_reg_data[request.body['username'].toLowerCase()] != 'undefined') {
            ...
            if (request.body['password'] == userdata.password) {
                ...
                response.redirect('./loginPage.html?' + stringify);
            }
        } else {
            response.send(` The username ${request.body['username']} or password ${request.body['password']} entered does not exist in our system. Please enter a valid login to proceed!`);
        }
    });
