...
app.post("/process_purchase", function(request, response) {
    ...
    if (typeof POST['submitPurchase'] != 'undefined') {
        ...
        for (i = 0; i < products.length; i++) {
            ...
            validQuantities = validQuantities && isNonNegInt(amount);
        }
...
//lab 13 reference
function isNonNegInt(amount, returnErrors = false) { //changed name to have more meaning
    errors = []; // assume no errors at first
    if (amount == "") { amount = 0; }
    if (Number(amount) != amount) errors.push('Not a number!'); // Check if string is a number value
    if (amount < 0) errors.push('Negative value!'); // Check if it is non-negative
    if (parseInt(amount) != amount) errors.push('Not an integer!'); // Check that it is an integer
